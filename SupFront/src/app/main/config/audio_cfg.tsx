...
export function formAttr(id_: string, param_name: string, param_value: string) {
    return {key: id_, attr_names: [param_name], attr_values: [param_value], attr_is_editable: [true]} as Row
}

export function UpdateRows(rows: Row[], inRow: Row) {
    let newRows: Row[] = []
    // 遍历属性列表，修改同key的一行属性的值
    rows.forEach(row => {
        let newRow: Row = row
        // 修改可编辑属性的值
        if (row.key == inRow.key) {
            row.attr_names.map((attr_name, idx) => {
                newRow.attr_names[idx] = attr_name
                newRow.attr_values[idx]= row.attr_values.at(idx) as string
                newRow.attr_is_editable[idx] = false

                if (row.attr_is_editable.at(idx)) {
                    newRow.attr_values[idx] = inRow.attr_values.at(idx) as string
